...
public class CcpSenderReceiverTest {
  ...
  public void setUp() {
...

    {
      ...
      final CcpReceiver ccpReceiver =
        new DefaulCcpReceiver(() -> connectorA_ConnectorSettings, incomingRouteForwardingRoutingTable,
          codecContext,
          pluginRunningOnA);
    ...
    final IpcRouteHandlingPlugin pluginRunningOnB = new IpcRouteHandlingPlugin(pluginSettingsA, codecContext);
    {
      ...
      final CcpSender ccpSender = new DefaultCcpSender(
        ...
        pluginRunningOnB);
      final CcpReceiver ccpReceiver =
        new DefaulCcpReceiver(() -> connectorB_ConnectorSettings, incomingRouteForwardingRoutingTable,
          codecContext,
          pluginRunningOnB);
  ...
  private void assertRoutingTableIsNotExpired(final SimulatedConnector... connectors) {
    Stream.of(connectors).forEach(connector ->
      assertThat(
        ((DefaulCcpReceiver) connector.ccpReceiver).getRoutingTableExpiry().isAfter(Instant.now()),
        is(true))
    );
  }
  ...
  private void assertRoutingTableIsExpired(final SimulatedConnector... connectors) {
    Stream.of(connectors).forEach(connector ->
      assertThat(
        ((DefaulCcpReceiver) connector.ccpReceiver).getRoutingTableExpiry()
          ...
          switch (sourcePreparePacket.getDestination().getValue()) {
            case CCP_CONTROL_DESTINATION: {
              ...
              return Completions.supplyAsync(
                () -> (InterledgerFulfillPacket) InterledgerFulfillPacket.builder()
                  .fulfillment(PEER_PROTOCOL_EXECUTION_FULFILLMENT).build())
                .toCompletableFuture();
            }
