        ...
        if (!plugin.isConnected()) {
          ...
          logger
            .error("Plugin emitted connect, but then returned false for isConnected, broken plugin. peerAccount={}",
              pluginConnectedEvent.getPeerAccountAddress());
          //setImmediate(() => this.add(accountId))
        } else {
          addAccount(pluginConnectedEvent.getPeerAccountAddress());
        }
      }

      @Override
      public void onDisconnect(PluginDisconnectedEvent pluginDisconnectedEvent) {
        removeAccount(pluginDisconnectedEvent.getPeerAccountAddress());
      }
      ...
      connectorSettingsSupplier.get().getRoutingSettings().getStaticRoutes().stream()
        ...
        .orElseGet(() -> {
            ...
            return localRoutingTable.getRouteByPrefix(addressPrefix)
              .orElseGet(() -> {
                // If we getEntry here, there was no local getRoute, so search through all Peers and sort all of the routes
