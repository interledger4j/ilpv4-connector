 ...
 *     purpose of this class is to define an interface that can be used for initiating a ping; however, the naming
 *     implies that implementors of this interface can accept and/or process ping requests, which is not the case.
 *     Second, ping requests should generally emanate from a Sender. While it's true that a sender might be using
 *     ILP-over-HTTP, the actual business logic of making a Ping request should not start at the link because, this link
 *     exists on both sides of a Connector, and could be used to circumvent Connector processing logic (e.g., grab an
 *     outbound link and "send a ping" will not trigger any balance-change logic on the source connector).
 */
@Deprecated
