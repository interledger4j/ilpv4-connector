 ...
 * address always maps to one account in a ledger, though it can also provide more specific information, such as an
 * invoice ID or a sub-account. Destination addresses MUST NOT end in a period (.) character. </p>
 *
 * <p><b>Address Prefixes</b> are incomplete addresses representing a grouping of destination
 * addresses. Many depths of grouping are possible, for example: groups of accounts or sub-accounts; an individual
 * ledger or sub-ledger; or entire neighborhoods of ledgers. Address prefixes MUST end in a period (.) character. </p>
 *
...
public interface InterledgerAddress {
  ...
  static InterledgerAddress of(final String value) {
    ...
    return ImmutableInterledgerAddress.builder().value(value).build();
  }
   ...
   *
   * @return {@code true} if the supplied {@code value} conforms to the requirements of RFC 15; {@code false} otherwise.
   *
   * @throws NullPointerException if {@code value} is <tt>null</tt>.
   */
   ...
   * example, if the value of this address is '<code>us.usd.</code>', then calling this method with an argument of
   * '<code>bob</code>' would result in a new Interledger Address with a value of '<code>us.usd.bob</code>', which is a
   * destination address.</p>
   *
   * <p>Likewise, if the value of this address is '<code>us.usd.pacific.</code>', then calling this
   * method with an argument of '<code>creditunions.</code>' would result in a new Interledger Address with a value of
   * '<code>us.usd.pacific.creditunions.</code>', which is an address prefix.</p>
   *
  ...
  /**
   * <p>Return this address's global-allocation-scheme prefix, which is a new {@link InterledgerAddress} containing the
   ...
   * containing 'g.'. Conversely, calling this method on an address that is already a global allocation scheme address,
   * like <tt>g</tt> would yield the same instance, for example <tt>g</tt>.</p>
   *
  ...
  default InterledgerAddress getPrefix() {
    return InterledgerAddress.of(getValue().substring(0, this.getValue().indexOf(ADDRESS_DELIMITER) + 1));
  }
   ...
   * InterledgerAddress} containing the characters inside of {@link #getValue()}, up-to and including last period. If
   * this address is instead a prefix, then this instance returns a new {@link InterledgerAddress} containing the
   * characters inside of {@link #getValue()}, up-to and including second-to-last period, unless this address is a root
   * prefix, in which case, {@link Optional#empty()} is returned.</p>
   *
   * <p>For example, calling this method on an address 'g.example.alice' would yield a new address
   * containing 'g.example.'. Likewise, calling this method on an address that is already a prefix, like 'g.example.'
   * would yield 'g.'. Finally, calling this method on a root prefix, like "self." would yield {@link
   * Optional#empty()}.</p>
   *
  ...
  default Optional<InterledgerAddress> getParentAddress() {
    ...
    if (isRootAddress()) {
      ...
      final String parentDestAddr = this.getValue().substring(0, this.getValue().lastIndexOf(ADDRESS_DELIMITER));
      return Optional.of(
        InterledgerAddress.of(parentDestAddr.substring(0, parentDestAddr.lastIndexOf(ADDRESS_DELIMITER))).getPrefix()
      );
    }
  }
   ...
   *
   * @return {@code true} if this address is a destination address. Otherwise (if this address is a prefix address),
   * then return {@code false} if this address is a Root prefix; otherwise, return {@code true}.
   */
  ...
  final class InterledgerAddressParser {
    ...
    private String getFirstInvalidityCause(final String invalidAddressString) {
      final List<String> schemeAndSegments = Arrays.asList(invalidAddressString.split(SEPARATOR_REGEX, -1));
