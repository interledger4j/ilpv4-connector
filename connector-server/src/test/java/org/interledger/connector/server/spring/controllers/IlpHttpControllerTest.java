...
public class IlpHttpControllerTest extends AbstractControllerTest {
  ...
  public void testApplicationOctetStream() throws Exception {
    postIlpPrepare(testOctetStreamHeaders(), status().isOk());
  }

  @Test
  public void testApplicationIlpOctetStream() throws Exception {
    postIlpPrepare(contentTypeHeader(APPLICATION_ILP_OCTET_STREAM_VALUE), status().isOk());
  }

  @Test
  public void testContentTypeCovfefe() throws Exception {
    postIlpPrepare(contentTypeHeader("application/covfefe"), status().isUnsupportedMediaType());
  }

  private void postIlpPrepare(HttpHeaders httpHeaders, ResultMatcher expectedStatus) throws Exception {
