...
import org.interledger.plugin.lpiv2.*;
...
import static com.sappenin.ilpv4.connector.ccp.CcpConstants.*;
...
public class CcpSenderReceiverTest {
  ...
  public void setUp() {
    ...
    final PluginSettings pluginSettingsA = ImmutablePluginSettings.builder()
      ...
      .peerAccount(CONNECTOR_B_ADDRESS)
...

    {
      ...
      final CcpSender ccpSender = new CcpSender.DefaultCcpSender(
      ...
      final CcpReceiver ccpReceiver =
        new CcpReceiver.DefaulCcpReceiver(() -> connectorA_ConnectorSettings, incomingRouteForwardingRoutingTable,
          codecContext,
          pluginRunningOnA);
    ...
    final PluginSettings pluginSettingsB = ImmutablePluginSettings.builder()
      .pluginType(PluginType.of(IpcRouteHandlingPlugin.class.getSimpleName()))
      .peerAccount(CONNECTOR_A_ADDRESS)
    ...
    final IpcRouteHandlingPlugin pluginRunningOnB = new IpcRouteHandlingPlugin(pluginSettingsA, codecContext);
    {
      ...
      final CcpSender ccpSender = new CcpSender.DefaultCcpSender(
        ...
        pluginRunningOnB);
      final CcpReceiver ccpReceiver =
        new CcpReceiver.DefaulCcpReceiver(() -> connectorB_ConnectorSettings, incomingRouteForwardingRoutingTable,
          codecContext,
          pluginRunningOnB);
  ...
  public void testMessageFlow() throws ExecutionException, InterruptedException, TimeoutException {
    ...
    // The Route to C
    ((CcpSender.DefaultCcpSender) this.connectorB.ccpSender).getForwardingRoutingTable().addRoute(
  ...
  private void assertRoutingTableEpoch(final int expectedEpoch, final SimulatedConnector... connectors) {
    Stream.of(connectors).forEach(connector ->
      assertThat(((CcpSender.DefaultCcpSender) connector.ccpSender).getForwardingRoutingTable().getCurrentEpoch(),
        is(expectedEpoch))
    );
  }
  ...
  private void assertSyncMode(final CcpSyncMode syncMode, final SimulatedConnector... connectors) {
    Stream.of(connectors).forEach(connector ->
      assertThat(((CcpSender.DefaultCcpSender) connector.ccpSender).getSyncMode(), is(syncMode))
    );
  }
  ...
  private void assertRoutingTableIsNotExpired(final SimulatedConnector... connectors) {
    Stream.of(connectors).forEach(connector ->
      assertThat(
        ((CcpReceiver.DefaulCcpReceiver) connector.ccpReceiver).getRoutingTableExpiry().isAfter(Instant.now()),
        is(true))
    );
  }
  ...
  private void assertRoutingTableIsExpired(final SimulatedConnector... connectors) {
    Stream.of(connectors).forEach(connector ->
      assertThat(
        ((CcpReceiver.DefaulCcpReceiver) connector.ccpReceiver).getRoutingTableExpiry()
  ...
  private void assertHasRouteForPrefix(final InterledgerAddressPrefix prefix, final SimulatedConnector... connectors) {
    Stream.of(connectors).forEach(connector ->
      assertThat(
        ((CcpSender.DefaultCcpSender) connector.ccpSender)
  ...
  private static class IpcRouteHandlingPlugin extends AbstractPlugin<PluginSettings> implements Plugin<PluginSettings> {
      ...
      this.registerDataHandler((preparePacket) -> {
        ...
        if (preparePacket.getDestination().startsWith("peer.getRoute")) {
          switch (preparePacket.getDestination().getValue()) {
            case CCP_CONTROL_DESTINATION: {
              ...
              logger
                .debug("Received getRoute control message. sender={}, tableId={} getEpoch={} features={}",
                  this.getPluginSettings().getPeerAccount(),
                  routeControlRequest.lastKnownRoutingTableId(), routeControlRequest.lastKnownEpoch(),
                  routeControlRequest.features());

              // Normally, we would consult the AccountManager to get the lpi2 for the sourceAccount. However, for
              ...
              // TODO: should handleRouteControlRequest return a completeable future?
              return InterledgerFulfillPacket.builder().fulfillment(PEER_PROTOCOL_EXECUTION_FULFILLMENT).build();
            }
            ...
            default:
              throw new Error("Unrecognized ccp message. destination=" + preparePacket.getDestination());
          }
        }
        throw new RuntimeException("Unhandled Destination: " + preparePacket.getDestination());
      });
    ...
    public InterledgerFulfillPacket doSendData(InterledgerPreparePacket preparePacket) throws InterledgerProtocolException {
      ...
      try {
        return this.remoteConnector.getPlugin().handleIncomingData(preparePacket).get();
     ...
     */
    @Override
    protected void doSendMoney(final BigInteger amount) {
