...
public interface CcpReceiver {
   ...
   */
  CompletableFuture<InterledgerResponsePacket> sendRouteControl();
   ...
   */
  class DefaulCcpReceiver implements CcpReceiver {
    ...
    public CompletableFuture<List<InterledgerAddressPrefix>> handleRouteUpdateRequest(
      ...
      routeUpdateRequest.newRoutes().stream()
        .map(ccpNewRoute -> ImmutableIncomingRoute.builder()
          .peerAddress(this.plugin.getPluginSettings().getPeerAccount())
    ...
    public CompletableFuture<InterledgerResponsePacket> sendRouteControl() {
      ...
      final InterledgerPreparePacket preparePacket = InterledgerPreparePacket.builder()
        ...
        .expiresAt(Instant.now().plus(
          connectorSettingsSupplier.get().getRouteBroadcastSettings(plugin.getPluginSettings().getPeerAccount())
            .getRouteExpiry())
        )
            ...
            if (error instanceof InterledgerProtocolException) {
              ...
              logger
                .error("Unknown response fulfillPacket type. peer={}",
                  plugin.getPluginSettings().getPeerAccount().getValue(), error);
            }
            return null;
          } else {
            logger.debug("Successfully sent getRoute control message. peer={}",
              plugin.getPluginSettings().getPeerAccount().getValue());
