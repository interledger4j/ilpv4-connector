...
/**
 * Represents a connection from a client node (using a {@link BtpClientPlugin}) to a server node that accepts BTPv2
 * connections.
 *
...
public class ClientWebsocketBtpPluginEdge extends Edge {
   ...
   * @param plugin
   */
  public ClientWebsocketBtpPluginEdge(final InterledgerAddress accountAddress, final Plugin plugin) {
    this(accountAddress.getValue(), plugin);
  }

  public ClientWebsocketBtpPluginEdge(final String nodeKey, final Plugin plugin) {
